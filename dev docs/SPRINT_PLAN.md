...
**Status:** S12 staging prep is in progress. `feature/s12-staging-prep` (UX/performance/a11y review, `service_role` grants fix, `/login` auth, proxy convention cleanup) is merged to `dev` (PR #21). Vercel Preview env vars and the `f1-league-manager-nonprod` Supabase link/migrations are done; remaining work is a stable `staging` domain, the real workbook smoke test, and official Lighthouse numbers against a live deployment.
...
| Staging Vercel environment variables | Done | 2026-07-04: set `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `CSRF_SECRET`, `NEXT_PUBLIC_SITE_URL` on the Vercel **Preview** scope, sourced from `f1-league-manager-nonprod`; confirmed via `vercel env ls` and a successful redeploy | Upstash values still unset (rate limiting fails safe/open per environment — see `src/lib/security/rate-limit.ts`; not yet required for current smoke-level testing). `NEXT_PUBLIC_SITE_URL` is a static value but Vercel gives every PR preview a unique URL, so CSRF-protected admin mutations (not public pages) will 403 on ad-hoc previews — needs a Vercel Custom Environment tied to the `staging` git branch with a fixed domain before real admin-flow smoke testing |
...
| Stable `staging` domain for CSRF-safe admin testing | Outstanding | None yet | Create a Vercel Custom Environment tied to the `staging` git branch with a fixed domain, and point its `NEXT_PUBLIC_SITE_URL` at that domain, before running admin-flow (not just public-page) smoke tests |
| Real workbook smoke on staging | Outstanding | Vercel + Supabase nonprod are now both provisioned, so this is unblocked | Owner to upload the Season 2 `.xlsx` via the admin import page against a live deploy and confirm a clean diff |
