...
function castDriver(v: unknown): { display_name: string; racing_number: number | null } | null {
  return v as { display_name: string; racing_number: number | null } | null;
}
function castTeam(v: unknown): { name: string; color_hex: string } | null {
  return v as { name: string; color_hex: string } | null;
}
    ...
    <section>
      ...
      <div className="hero-band">
        ...
        <div className="hero-band__content">
          ...
          {nextRace ? (
            <RaceCountdown targetIso={nextRace.scheduled_at} />
          ) : (
              ...
              {topDrivers.map((row) => {
                ...
                return (
                  <li key={row.position} className="flex items-center gap-3 border border-f1-border bg-f1-dark px-4 py-2">
                    ...
                    />
                    <span className="flex-1 text-sm text-f1-white">
                      {driver?.display_name ?? "—"}
                  ...
                  return (
                    <li key={row.position} className="flex items-center gap-3 border border-f1-border bg-f1-dark px-4 py-2">
                      ...
                      <span
                        ...
                        style={{ backgroundColor: team?.color_hex ?? "#444" }}
                      />
                      <span className="flex-1 text-sm text-f1-white">{team?.name ?? "—"}</span>
                      <span className="font-mono text-sm text-f1-white">{row.total_points} pts</span>
                      <span className="w-16 text-right font-mono text-xs text-f1-muted">{gap}</span>
                    </li>
                  );
